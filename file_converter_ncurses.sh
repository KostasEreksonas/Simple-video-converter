...
# Restart a sequence of settings prompts if some are incorrect
check=1
while [ $check -eq 1 ]; do
	...
	while ! [[ $crf =~ ^[0-9]*$ && $crf -ge 0 && $crf -le 53 ]]; do
		crf=$(dialog --stdout --title "Choose CRF Value" \
			--inputbox "Choose CRF value (between 0 - 53, default: 17)" 0 0)
		if [ -z $crf ]; then crf="${1:-17}"; fi
	done

	dialog --title "Settings" --yesno "Please confirm your settings:\n\
	Audio: $audio\nVideo: $video\nPreset: $preset\nCRF: $crf\n\
	Would you like to continue with these settings? (Yes/No)" 0 0

	check=$? && if [ $check -eq 0 ]; then break; fi
done
